...
char xor_header(char* buff, int sz)
{
	char c = 0;
	int i = 0;
	for(;i < sz; ++i)
...
int xor_data(char* inoutBuf, int sz, char magic1,char magic2)
{
	int i = 0;
	for (; i < sz; ++i)
	{
...
static inline int
mptun_encrypt_impl(const char in[BUFF_SIZE], int sz, char out[BUFF_SIZE], uint64_t key, time_t ti) {
	...
	retsz = xor_data(out, sz + 8, xor_magic_1, xor_magic_2);
...
static inline int
mptun_decrypt_impl(char in[BUFF_SIZE], int sz, char out[BUFF_SIZE], uint64_t key, time_t ti) {
	uint32_t pt, check;
	uint64_t h;
	struct rc4_sbox rs;
	
	xor_data(in, sz, xor_magic_2, xor_magic_1);
...
add_remote(struct tundev *tdev, SOCKADDR *addr, int bytes) {
	...
	if (g_log > 0)
	{
		sprintf(buff, "remote num:%d", tdev->remote_n);
		Log(buff);
	}
...
ifconfig(const char * ifname, const char * va, const char *pa) {
	...
	// snprintf(cmd, sizeof(cmd), "ip -4 addr flush dev %s scope global", ifname);
	// if(system(cmd) < 0) exit(1);
	// snprintf(cmd, sizeof(cmd), "ip -4 addr add dev %s %s/%d broadcast + scope global", ifname, va, 32);
	// if(system(cmd) < 0) exit(1);
